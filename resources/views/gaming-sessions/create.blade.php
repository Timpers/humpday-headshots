...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Create Gaming Session</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-2">Schedule a gaming session and invite friends or groups</p>
    </div>

    <form action="{{ route('gaming-sessions.store') }}" method="POST" class="space-y-8">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="relative">                        <input type="text" 
                        ...
                        <input type="hidden" name="game_name" id="game_name" value="{{ old('game_name') }}">
                    ...
                    @error('game_name')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        💡 Start typing to search IGDB database, or enter any game name manually
                    </p>
                    
                    <!-- Selected Game Display -->
                    <div id="selected_game" class="mt-3 {{ old('game_name') ? '' : 'hidden' }}">
                        <div class="flex items-center p-3 bg-blue-50 dark:bg-blue-900/20 rounded-md">
                            ...
                            <div>
                                <p id="selected_game_name" class="font-medium text-gray-900 dark:text-white">{{ old('game_name') }}</p>
                ...
                <div>
                    ...
                    <select name="platform" 
                        ...
                        @foreach(\App\Models\Game::PLATFORMS as $value => $label)
                            <option value="{{ $value }}" {{ old('platform') === $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="privacy" 
                        ...
                        <option value="friends_only" {{ old('privacy', 'friends_only') === 'friends_only' ? 'selected' : '' }}>
                            Friends Only
                        </option>
                        <option value="invite_only" {{ old('privacy') === 'invite_only' ? 'selected' : '' }}>
                            Invite Only
                        </option>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" 
                              ...
                              class="w-full border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="requirements" 
                              ...
                              class="w-full border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white @error('requirements') border-red-500 @enderror">{{ old('requirements') }}</textarea>
                    @error('requirements')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <!-- Invitations -->
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-6">Send Invitations</h3>
                        ...
                        <div class="space-y-2 max-h-48 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-md p-3">
                            @foreach($friends as $friend)
                                <label class="flex items-center">
                                    ...
                                    <span class="ml-2 text-sm text-gray-900 dark:text-white">{{ $friend->name }}</span>
                                </label>
                            @endforeach
                        </div>
                    @else
                        <p class="text-gray-500 dark:text-gray-400 text-sm">You haven't connected with any friends yet.</p>
                    @endif
                </div>
                            ...
                            @foreach($groups as $group)
                                <label class="flex items-center">
                                    <input type="checkbox" 
                                           ...
                                           value="{{ $group->id }}"
                                           class="rounded border-gray-300 dark:border-gray-600 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                    <span class="ml-2 text-sm text-gray-900 dark:text-white">{{ $group->name }}</span>
                                </label>
                            @endforeach
                        </div>
                    @else
                        <p class="text-gray-500 dark:text-gray-400 text-sm">You're not a member of any groups yet.</p>
                    @endif
                </div>
            </div>
        </div>
